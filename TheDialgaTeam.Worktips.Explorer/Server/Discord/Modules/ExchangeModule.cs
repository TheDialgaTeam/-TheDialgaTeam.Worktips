    ...
    HttpClient httpClient) : InteractionModuleBase<InteractionContext>
{
    ...
    public async Task PriceCommand(
        [Choice("Litecoin (Tradeogre)", "LTC")] [Choice("Bitcoin (Tradeogre)", "BTC")] [Summary("type", "Type of price to retrieve.")]
        string type = "LTC")
    {
        await DeferAsync().ConfigureAwait(false);
        
        var ticker = await httpClient.GetFromJsonAsync($"https://tradeogre.com/api/v1/ticker/{_blockchainOptions.CoinTicker}-{type}", TickerContext.Default.Ticker).ConfigureAwait(false);
